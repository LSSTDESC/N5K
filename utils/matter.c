...
int matter_init(
  ...
  pma->ptw_size = 800;//800;//1200;//800;
  //IMPORTANT TO BE HIGH ptw size
  pma->ptw_integrated_size = 1600;//1600;//1600;//160;
  ...
  pma->k_max_extr = 1000.0;

  pma->k_weight_k_max = 10.0;
  pma->k_weight_k_min = 0.01;
  pma->k_weight_mode = matter_k_weights_gaussian;
  ...
  class_alloc(k_weights,
              ...
              pma->error_message);
  class_call(matter_obtain_growth_factor_k_weights(pma,k_weights),
             pma->error_message,
             pma->error_message);
...
int matter_obtain_k_sampling(
   ...
   * */
  k_min = pma->k_min_file;
  pma->k_max = pma->k_file[pma->k_size_file-1];
  ...
  pma->k_sampling[0] = pma->k_min_file;
...
int matter_obtain_time_sampling(
  ...
  for(index_wd=0;index_wd<pma->num_windows;++index_wd){
     ...
     * Find tau_min
     * */
    if (pma->selection==gaussian) {
      zmin = pma->selection_mean[index_wd]+pma->selection_width[index_wd]*pma->selection_cut_at_sigma;
    }
    if (pma->selection==tophat) {
      zmin = pma->selection_mean[index_wd]+(1.+pma->selection_cut_at_sigma*pma->selection_tophat_edge)*pma->selection_width[index_wd];
    }
    if (pma->selection==dirac) {
      zmin = pma->selection_mean[index_wd]+pma->small_log_offset;
    }
     ...
     * */
    if (pma->selection==gaussian) {
      zmax = pma->selection_mean[index_wd]-pma->selection_width[index_wd]*pma->selection_cut_at_sigma;
    }
    if (pma->selection==tophat) {
      zmax = pma->selection_mean[index_wd]-(1.+pma->selection_cut_at_sigma*pma->selection_tophat_edge)*pma->selection_width[index_wd];
    }
    if (pma->selection==dirac) {
      zmax = pma->selection_mean[index_wd]-pma->small_log_offset;
    }
    ...
    if(pma->matter_verbose > MATTER_VERBOSITY_RANGES){
      printf(" -> Obtained limits %f to %f for window %i, \n \
              with mean = (z: %f, chi: %f) and width (z: %f, chi: %f) \n",tau_min,tau_max,index_wd,pma->selection_mean[index_wd],pma->tau0-(tau_max+tau_min)*0.5,pma->selection_width[index_wd],(tau_max-tau_min)*0.5/pma->selection_cut_at_sigma);
    }
...
int matter_obtain_prepare_windows_parallel(
  ...
  for(index_wd=0;index_wd<pma->num_windows;++index_wd){
    for(index_ptw=0;index_ptw<pma->ptw_size;index_ptw++){
      ...
      z = pma->a_today/pma->short_pvecback[pma->index_bg_a]-1.;
      ...
      pma->ptw_orig_window[index_wd*pma->ptw_size+index_ptw] = window_at_z*pma->short_pvecback[pma->index_bg_H]*pma->a_today;
    }
    /**
    ...
    if((pma->has_nz_evo_file == _TRUE_) || (pma->has_nz_evo_analytic == _TRUE_)){
      ...
      for(index_wd=0;index_wd<pma->num_windows;++index_wd){
        for(index_ptw=0;index_ptw<pma->ptw_size;++index_ptw){
           ...
           * */
          f_evo[index_wd*pma->ptw_size+index_ptw] = 2./pma->short_pvecback[pma->index_bg_H]/pma->short_pvecback[pma->index_bg_a]/(pma->tau0-tau)
            + pma->short_pvecback[pma->index_bg_H_prime]/pma->short_pvecback[pma->index_bg_H]/pma->short_pvecback[pma->index_bg_H]/pma->short_pvecback[pma->index_bg_a];
          ...
          f_evo[index_wd*pma->ptw_size+index_ptw] -= dln_dNdz_dz/pma->short_pvecback[pma->index_bg_a];
        }
        //End ptw
      }
      //End wd
  ...
  for(n_thread=0;n_thread<N_threads;++n_thread){
    class_alloc(pvecback[n_thread],
                pma->bg_size_short*sizeof(double),
                pma->error_message);
  }
    ...
    for(index_radtp=0;index_radtp<pma->radtp_size_total;++index_radtp){
      ...
      if(
        ...
        for(index_wd=0;index_wd<pma->num_windows;++index_wd){
          ...
          int tid = 0;
#endif
          class_call_parallel(matter_integrate_window_function(
                                           pma,
          ...
          for(index_ptw_integrated=0;index_ptw_integrated<pma->ptw_integrated_size;++index_ptw_integrated){
            ...
            a * pma->growth_factor_tau[inf] +
            b * pma->growth_factor_tau[inf+1] +
            ((a*a*a-a)* pma->ddgrowth_factor_tau[inf] +
             (b*b*b-b)* pma->ddgrowth_factor_tau[inf+1])*h*h/6.;
          }
          //End ptw_integrated
        }
        //End wd
        ...
        for(index_wd=0;index_wd<pma->num_windows;++index_wd){
          ...
          for(index_ptw=0;index_ptw<pma->ptw_size;++index_ptw){
            ...
            prefactor_dop1 = (
                    1.0 +
                    pvecback[tid][pma->index_bg_H_prime]/(pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H]*pvecback[tid][pma->index_bg_H]) +
                    (2.0-5.0*pma->selection_magnification_bias[index_wd])/(pma->tau0-tau)/(pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H]) +
                    5*pma->selection_magnification_bias[index_wd] -
                    f_evo[index_wd*pma->ptw_size+index_ptw]
                  );
            prefactor_dop2 = (
                  (f_evo[index_wd*pma->ptw_size+index_ptw]-3.0)*pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H]
                  );
            prefactor_rsd = (
                    1.0/(pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H])
                  );
               ...
               * */
                ...
                if(matter_is_index(index_stp_combined,pma->stp_index_psi,pma->has_gravitational_terms)){
                  prefactor = pma->relative_factors[index_stp_combined]*(
                    2.0 + pvecback[tid][pma->index_bg_H_prime]/(pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H]*pvecback[tid][pma->index_bg_H])
                    + (2.0-5.0*pma->selection_magnification_bias[index_wd])/pvecback[tid][pma->index_bg_a]/pvecback[tid][pma->index_bg_H]/(pma->tau0-tau)
                  ...
                  prefactor = pma->relative_factors[index_stp_combined]/(pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H]);
                }
                else{
        ...
        for(index_wd=0;index_wd<pma->num_windows;++index_wd){
          ...
          for(index_ptw=0;index_ptw<pma->ptw_size;++index_ptw){
            ...
            if(matter_is_index(index_radtp,pma->radtp_dens,pma->has_stp_delta_m && (!pma->uses_density_splitting))){
              ...
              prefactor = 1.0/(pvecback[tid][pma->index_bg_a]*pvecback[tid][pma->index_bg_H]);
            }
            else{
...
int matter_window_function(
  ...
  x=fabs(z-pma->selection_mean[bin]);

  /** gaussian case */
  if (pma->selection==gaussian) {

    *window_at_z = exp(-0.5*pow(x/pma->selection_width[bin],2))
      /pma->selection_width[bin]/sqrt(2.*_PI_);
  ...
  if (pma->selection==tophat) {
    ...
    /* selection function, centered on z=mean (i.e. on x=0), equal to
       ...
       delta x = 0.1*width
    */
    *window_at_z=(1.-tanh((x-pma->selection_width[bin])/(pma->selection_tophat_edge*pma->selection_width[bin])))/2.;
...
int matter_obtain_primordial_spectrum(
  ...
  int index_coeff;
  int index_ic1_ic2;
  int index_md = 0; //pma->index_md_scalars;
  if(pma->matter_verbose > MATTER_VERBOSITY_RANGES) {
...
int matter_obtain_growth_factor_k_weights(
  ...
  if(pma->k_weight_mode == matter_k_weights_gaussian){
    ...
    for(index_k = 0; index_k < pma->k_size; ++index_k){
      double log_k = log(pma->k_file[index_k]);
  ...
  for (index_ic = 0; index_ic < pma->ic_size; index_ic++) {
    for (index_stp = 0; index_stp < pma->stp_size; index_stp++) {
      ...
      for(index_tau = 0; index_tau < pma->tau_size;++index_tau){
        pma->growth_factor_tau[index_tau] = 0.0;
        for(index_k = 0; index_k < pma->k_size; ++index_k){
          pma->growth_factor_tau
                  [index_tau] +=
          k_weights[index_k] *
...
int matter_extrapolate_sources(
  ...
  *k_extrapolated=NULL;
  ...
  /*class_call(get_extrapolated_source_size(
                                          pma->k_per_decade_for_pk,
                                          pma->k_max,
                                          pma->k_max_extr,
                                          pma->k_size[pma->index_md_scalars],
                                          &k_size_extr,
                                          pma->error_message),
             pma->error_message,
             pma->error_message);*/
  ...
  class_alloc(*k_extrapolated,
              ...
              pma->error_message);
  /*class_call(extrapolate_k(pma->k[pma->index_md_scalars],
                           pma->k_size,
                           *k_extrapolated,
                           pma->k_per_decade_for_pk,
                           pma->k_max_extr,
    ...
    for(index_stp=0;index_stp<pma->stp_size;++index_stp){
      ...
      for(index_tau=0;index_tau<pma->tau_size;++index_tau){
        /*class_call(extrapolate_source(*k_extrapolated,
                                      pma->k_size,
                                      k_size_extr,
                                      sources[index_ic*pma->stp_size+index_stp]+index_tau*pma->k_size[pma->index_md_scalars],
                                      extrapolation_type,
 ...
 * @return the error status
 */
int matter_integrate_window_function(struct matters* pma,
                                     double* window,
  ...
  if(matter_is_index(radtp,pma->radtp_g5,pma->has_gravitational_terms)){
    ...
    for(index_oldtw=0;index_oldtw<oldtw_size;++index_oldtw){
      /*class_call(background_at_tau(pba,
                                   ...
                                   pma->short_info,
                                   pma->inter_closeby, //pma->inter_closeby
                                   &last_index,
                                   pvecback),
               pma->error_message,
               pma->error_message);*/
      bg_factor[index_oldtw] = 2.0*(1.0 + pvecback[pma->index_bg_H_prime]
                    /pvecback[pma->index_bg_a] / pvecback[pma->index_bg_H] / pvecback[pma->index_bg_H]
                    + (2.0 - 5.0 *pma->selection_magnification_bias[index_wd])
                    /(pma->tau0-oldtw_sampling[index_oldtw])/ pvecback[pma->index_bg_a] / pvecback[pma->index_bg_H]
      ...
      for( index_wd = 0; index_wd < pma->num_windows ; ++index_wd){
        for( index_tw=0;index_tw<pma->tw_size;++index_tw){
          class_call(matter_interpolate_spline_growing_hunt(
                          ...
                          pma->tau_size,
                          pma->growth_factor_tau,
                          pma->ddgrowth_factor_tau,
                          1,
                          pma->tw_sampling[index_wd*pma->tw_size+index_tw],
                          &last_index,
                          &pma->growth_factor[index_wd*pma->tw_size+index_tw],
                          pma->error_message),
                     pma->error_message,
                     pma->error_message);
        }
