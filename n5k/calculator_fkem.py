...
from fftlogx.fftlogx import fftlog
import sys
...
class N5KCalculatorFKEM(N5KCalculatorBase):
    ...
    def setup(self):
        ...
        dpk = self.get_pk()
        a = 1./(1+dpk['z'][::-1])
        pname = 'delta_matter:delta_matter'
        cp = {'Omega_c': par['Omega_m']-par['Omega_b'],
              'Omega_b': par['Omega_b'],
              'h': par['h'], 'n_s': par['n_s'],
              ...
              'pk_nonlin': {'a': a,
                            'k': dpk['k'],
                            pname: dpk['pk_nl'][::-1][:]}}
        self.cosmo = ccl.CosmologyCalculator(**cp)
        cp['pk_nonlin'][pname] = (dpk['pk_nl']-dpk['pk_lin'])[::-1][:]
        self.cosmo_nonlin_part = ccl.CosmologyCalculator(**cp)

        chi_min = ccl.comoving_radial_distance(cosmo=self.cosmo,
                                               a=1./(1.+0.002))
        chi_max = ccl.comoving_radial_distance(cosmo=self.cosmo,
                                               a=1./(1.+4.))
        Nchi = 200
        self.chi_logspace_arr = np.logspace(np.log10(chi_min),
                                            np.log10(chi_max),
                                            num=Nchi,
                                            endpoint=True)
        ...
        self.k_in = dpk['k']
        self.pklin_interp = interp1d(np.log(self.k_in), np.log(self.plin0),
        ...
        if self.config.get('tracers_from_kernels', False):
            ...
            for k in ker['kernels_cl']:
                ...
                ker_at_chi = interp1d(np.log(ker['chi_cl']), k,
                                      bounds_error=False, fill_value=0.)
                ker_logspace = ker_at_chi(np.log(self.chi_logspace_arr)) * \
                    self.chi_logspace_arr * growfac_arr
            ...
            for k in ker['kernels_sh']:
                ...
                ker_at_chi = interp1d(np.log(ker['chi_sh']), k,
                                      bounds_error=False, fill_value=0.)
                ker_logspace = ker_at_chi(np.log(self.chi_logspace_arr)) * \
                    self.chi_logspace_arr * growfac_arr
    ...
    def _get_cl_limber_nonlin_part(self, t1, t2, ls):
        return ccl.angular_cl(self.cosmo_nonlin_part, t1, t2, ls,
                              limber_integration_method='qag_quad')
    ...
    def _get_cls_nonlimber_lin(self, fchi1, fchi2, ls, cl_type, i1, i2):
        ...
        if (cl_type == "gg"):
            if (self.fk_g[i1] is None):
                myfftlog1 = fftlog(self.chi_logspace_arr, fchi1, nu=nu,
                                   N_extrap_low=0, N_extrap_high=0,
                                   c_window_width=0.25, N_pad=Npad)
                self.k_logspace_arr, self.fk_g[i1] = myfftlog1.fftlog_ells(ls)
            fk1 = self.fk_g[i1]

            if (self.fk_g[i1+i2] is None):
                myfftlog2 = fftlog(self.chi_logspace_arr, fchi2, nu=nu2,
                                   N_extrap_low=0, N_extrap_high=0,
                                   c_window_width=0.25, N_pad=Npad2)
                self.k_logspace_arr, self.fk_g[i1+i2] = \
                    myfftlog2.fftlog_ells(ls)
            fk2 = self.fk_g[i1+i2]
        ...
        if (cl_type == "gs"):
            ...
            if (self.fk_s[i2] is None):
                myfftlog2 = fftlog(self.chi_logspace_arr, fchi2, nu=nu,
                                   N_extrap_low=0, N_extrap_high=0,
                                   c_window_width=0.25, N_pad=Npad)
                self.k_logspace_arr, self.fk_s[i2] = \
                    myfftlog2.fftlog_modified_ells(ls)
            fk2 = self.fk_s[i2]
        ...
        if (cl_type == "ss"):
            ...
            if (self.fk_s[i1] is None):
                myfftlog1 = fftlog(self.chi_logspace_arr, fchi1, nu=nu,
                                   N_extrap_low=0, N_extrap_high=0,
                                   c_window_width=0.25, N_pad=Npad)
                self.k_logspace_arr, self.fk_s[i1] = \
                    myfftlog1.fftlog_modified_ells(ls)
            fk1 = self.fk_s[i1]
            ...
            if (self.fk_s[i1+i2] is None):
                myfftlog2 = fftlog(self.chi_logspace_arr, fchi2, nu=nu,
                                   N_extrap_low=0, N_extrap_high=0,
                                   c_window_width=0.25, N_pad=Npad)
                self.k_logspace_arr, self.fk_s[i1+i2] = \
                    myfftlog2.fftlog_modified_ells(ls)
            fk2 = self.fk_s[i1+i2]

        k = self.k_logspace_arr
        cls_nonlimber_lin = np.sum(fk1 * fk2 * k**kpow *
                                   np.exp(self.pklin_interp(np.log(k))),
                                   axis=1) * self.dlnr * 2./np.pi
    ...
    def _get_cl_nonlimber_fang(self, t1, t2, ls, i1, i2, cl_type):
        ...
        if (cl_type == 'gg'):
            ...
            cls_limber_nonlin_part = self._get_cl_limber_nonlin_part(t1, t2,
                                                                     ls_nonlim)
            cls_nonlimber_lin = self._get_cls_nonlimber_lin(self.t_g_logspace[i1],
                                                            self.t_g_logspace[i1+i2],
                                                            ls_nonlim, cl_type, i1, i2)
        ...
        if (cl_type == 'ss'):
            ...
            cls_limber_nonlin_part = self._get_cl_limber_nonlin_part(t1, t2,
                                                                     ls_nonlim)
            cls_nonlimber_lin = self._get_cls_nonlimber_lin(self.t_s_logspace[i1],
                                                            self.t_s_logspace[i1+i2],
                                                            ls_nonlim, cl_type, i1, i2)
            cls_nonlimber = cls_limber_nonlin_part + cls_nonlimber_lin
            cls_limber = self._get_cl_limber(t1, t2, ls[ls >= l_switch])
            return np.hstack((cls_nonlimber, cls_limber))
    ...
    def run(self):
        ...
        for i1, t1 in enumerate(self.t_g):
            for i2, t2 in enumerate(self.t_g[i1:]):
                self.cls_gg.append(self._get_cl_nonlimber_fang(t1, t2, ls,
                                                               i1, i2, 'gg'))
            for i2, t2 in enumerate(self.t_s):
                self.cls_gs.append(self._get_cl_nonlimber_fang(t1, t2, ls,
                                                               i1, i2, 'gs'))
        for i1, t1 in enumerate(self.t_s):
            for i2, t2 in enumerate(self.t_s[i1:]):
                self.cls_ss.append(self._get_cl_nonlimber_fang(t1, t2, ls,
                                                               i1, i2, 'ss'))
